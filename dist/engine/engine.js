import { baseCardValue, buildStandardDeck, isTenValueCard, shuffleDeck, } from "./deck.js";
...
export class BlackjackEngine {
    ...
    beginDealerTurn(playDealerHand) {
    ...
    resolveOutcome(input) {
        const { hand, playerScore, playerNatural, dealerScore, dealerNatural, dealerBusted, } = input;
    ...
    resolvePayoutReturned(hand, outcome) {
        if (outcome === "lose") {
            return 0;
        }
        if (outcome === "push") {
    ...
    canSplit(hand) {
        const [firstCard, secondCard] = hand.cards;
        const defaultCanSplit = hand.cards.length === 2 &&
    ...
    canDouble(hand) {
        const defaultCanDouble = hand.cards.length === 2 &&
            !hand.hasActed &&
            this.state.bankroll >= hand.wager;
        const context = {
            state: this.getState(),
    ...
    getBlackjackPayoutMultiplier(hand) {
        const context = {
            state: this.getState(),
            hand: this.cloneHand(hand),
            dealerHand: this.cloneCards(this.state.dealerHand),
    ...
    applyTargetScore(baseTargetScore) {
        const context = this.createRoundContext(baseTargetScore);
    ...
    applyCardValueModifiers(baseValue, context) {
    ...
    applyHandScoreModifiers(baseScore, context) {
    ...
    createRoundContext(targetScore) {
        return {
            state: this.getState(),
            roundNumber: this.state.roundNumber,
            bankroll: this.state.bankroll,
    ...
    isNaturalDealerBlackjack() {
        return this.isNaturalCards(this.state.dealerHand, "dealer", false);
    }
    isNaturalCards(cards, owner, allowTenValueCard) {
        if (cards.length !== 2) {
            return false;
        }
    ...
    createHand(cards, wager, isFromSplit) {
        const hand = {
            id: this.nextHandId,
