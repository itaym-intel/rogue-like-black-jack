...
export async function runCliGame(options = {}) {
    ...
    try {
        while (true) {
            ...
            if (state.phase === "awaiting_bet" || state.phase === "round_settled") {
                ...
                const answer = (await rl.question(`Bankroll ${formatMoney(state.bankroll)}. Enter wager (min ${engine.getMinimumBet()}) or q: `))
                    .trim()
                    .toLowerCase();
                if (isQuit(answer)) {
                    break;
...
async function runShopPhase(rl, manager) {
    const meta = manager.getMetaState();
    ...
    while (true) {
        ...
        const answer = (await rl.question("Enter item number to buy, (i)nventory, or (l)eave shop: "))
            .trim()
            .toLowerCase();
        if (isQuit(answer)) {
            manager.leaveShop();
...
function printShopOfferings(offerings, bankroll) {
    console.log("Items for sale:");
    offerings.forEach((offering, index) => {
        const affordable = bankroll >= offering.price ? "" : " (can't afford)";
        console.log(`  ${index + 1}. [${offering.item.itemRarity.toUpperCase()}] ${offering.item.itemName} — ${formatMoney(offering.price)}${affordable}`);
...
function printInventory(manager) {
    ...
    if (inventory.isEmpty()) {
        ...
        inventory.getItems().forEach((item, index) => {
            console.log(`  ${index + 1}. [${item.itemRarity.toUpperCase()}] ${item.itemName} — ${item.itemDescription}`);
        });
    }
    console.log("");
}
function printMetaStatus(meta, bankroll) {
    ...
    console.log(`Hands: ${meta.handsPlayed} | Stage: ${meta.stage} | Hands until next stage: ${handsRemaining} | Need ${formatMoney(nextThreshold)} to clear next stage`);
}
function printActiveRound(engine, state, meta) {
    console.log("");
    ...
    state.playerHands.forEach((hand, index) => {
        ...
        console.log(`${isActive} Hand ${index + 1} | Bet ${formatMoney(hand.wager)} | Score ${score} | ${cards}${status}`);
    });
}
function printRoundSummary(summary) {
    console.log("");
    console.log(`Round ${summary.roundNumber} resolved`);
    console.log(`Bankroll: ${formatMoney(summary.bankrollBeforeRound)} -> ${formatMoney(summary.bankrollAfterRound)}`);
    console.log(`Dealer: ${summary.dealerCards.map(formatCard).join(" ")} | Score ${summary.dealerScore}${summary.dealerBusted ? " (BUST)" : ""}`);
    summary.handResults.forEach((result, index) => {
        console.log(`Hand ${index + 1}: ${result.outcome.toUpperCase()} | Score ${result.score} | Bet ${formatMoney(result.wager)} | Returned ${formatMoney(result.payoutReturned)} | ${result.cards.map(formatCard).join(" ")}`);
    });
}
function printStageFailure(meta) {
    if (meta.stageMoneyThreshold > 0) {
        console.log(`Failed to meet Stage ${meta.stage} requirement of ${formatMoney(meta.stageMoneyThreshold)}.`);
    }
}
